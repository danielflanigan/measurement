...
from measurement.io import netcdf, npyjson
...
def run_loaded_sweep(ri, length_seconds=0, state=None, description='', tone_bank_indices=None, bin_indices=None,
    ...
    stream_arrays = core.MeasurementList()
    if verbose:
        print("Measuring bank:", end='')
    for tone_bank_index in tone_bank_indices:
        if verbose:
            print(tone_bank_index, end='')
            sys.stdout.flush()
...
def git_log():
    import measurement
    measurement_directory = os.path.dirname(os.path.abspath(measurement.__file__))
    try:
        return subprocess.check_output(("cd {}; git log -1".format(measurement_directory)), shell=True)
    except Exception as e:
        return str(e)
...
def git_status():
    ...
    measurement_directory = os.path.dirname(os.path.abspath(measurement.__file__))
    try:
        return subprocess.check_output(("cd {}; git status --porcelain".format(measurement_directory)), shell=True)
    except Exception as e:
        return str(e)
...
def get_script_logger(name, level=logging.INFO):
    script_logger = logging.getLogger('measurement')
