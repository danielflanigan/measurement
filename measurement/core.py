"""
This module contains the core classes of the measurement package:
- Node
- Measurement
- MeasurementList
- IOList
...
- StateDict
- IO
It also contains some functions for handling paths in the hierarchical files.

See __init__.py for the main package documentation.
"""
...
class StateDict(dict):
    ...
    union of the restrictions imposed by the libraries used to write non-array data to disk:
    - basic types, such as numbers, booleans, strings, booleans, and None;
    - sequences, i.e. lists, tuples, and arrays, that contain exactly one of the above basic types except None;
      sequences cannot contain other sequences;
    ...
    Two values that cause problems are None and NaN. None requires a special case because netCDF4 cannot write it,
    so it is allowed as an attribute value or dictionary value but not in sequences. NaN is stored and retrieved
    correctly but is not recommended as an indicator of missing state because it causes comparisons to fail:
    float('NaN') == float('NaN') is always False. See Measurement.__eq__() for more.
